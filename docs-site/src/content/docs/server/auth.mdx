...
```bash
curl http://localhost:4000/api/intents \
...
```bash
curl -X POST http://localhost:4000/api/auth/keys \
...
```bash
curl http://localhost:4000/api/auth/keys \
  -H "X-Api-Key: mr_a1b2c3d4e5f6..."
```
...
```bash
curl -X DELETE http://localhost:4000/api/auth/keys/production \
  -H "X-Api-Key: mr_a1b2c3d4e5f6..."
```
