---
title: MicroResolve
description: Pre-LLM reflex layer — deterministic intent classification at ~30µs, CPU-only, continuous learning.
template: splash
hero:
  tagline: A pre-LLM reflex layer that classifies intent, detects PII, and filters tool calls in ~30µs — before the LLM ever sees the query.
  actions:
...
```
query → [MicroResolve ~30µs] → matched intent → template / tool / guardrail
