...
engine = Engine(data_dir="~/.local/share/microresolve")
ns = engine.namespace("support")
...
<Aside type="tip">
For the full method listing see the [Python API Reference](/python/api/).
</Aside>

## Next

- [Python API Reference](/python/api/) — complete method listing
- [Connect to server](/python/connect/) — sync a local engine with the self-hosted server
