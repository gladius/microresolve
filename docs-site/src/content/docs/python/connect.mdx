...
def classify(query: str) -> list[dict]:
    r = httpx.post(f"{BASE}/api/resolve",
                   headers=HEADERS,
                   json={"query": query})
    r.raise_for_status()
...
For the lowest possible classification latency, run a local engine and treat the server as the training hub. Use the Rust library's built-in connected mode for this pattern — see [Rust — Connect to Server](/microresolve/rust/connect/) for details.
