...
For a per-call override, use `resolve_with_options` (lower-level; prefer setting per-namespace threshold):

```rust
let (result, _trace) = ns.resolve_with_options(
    query,
    Some(1.3),  // threshold_override
    2.0,        // gap
    0.3,        // fallback_threshold
    false,      // with_trace
);
```
