...
The embedded `Engine` runs entirely in-process. For teams that want a shared training pipeline, you can call the self-hosted server directly over HTTP.
...
For the lowest possible latency, run a local engine in-process and use the server as the training hub. Built-in connected mode is available in the Rust library — see [Rust — Connect to Server](/rust/connect/).
