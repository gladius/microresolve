...
```
Query → L0 (char-ngram typo correction)
      → L1 (LexicalGraph: morphology + abbreviation rewriting; synonym
            and semantic edges stored for inspection only, not injected)
      → L2 (IntentIndex: IDF-weighted sparse word→intent scoring with
            asymptotic Hebbian learning + token consumption for multi-intent)
      → confidence/disposition + cross-provider disambiguation
```

All layers owned by `Engine` + `NamespaceHandle` in the library. No
server-side sync required for embedded use. Learning =
`namespace.correct(query, wrong, right)` → atomic L2 weight update.
