...
def main():
    ...
    print(f"  {args.rounds} learning rounds | metric: ALL expected intents in top-N")
    print("=" * 80)
    print(f"\n  {'Round':<12} {'Top-1':>7}  {'Top-3':>6}  {'Top-5':>6}  {'Exact':>6}  {'Partial':>7}  {'Corrections':>6}  {'p50µs':>7}")
    print(f"  {'-'*76}")
    ...
    for rnd in range(args.rounds + 1):
        ...
        print_metrics(label, results, total_corrections)

        if rnd < args.rounds:
            corrections = 0
            for r in results:
                if r["missed"]:
                    spans = auto_learn(NS, r["text"],
                                       list(r["confirmed"]),
                                       list(r["expected"]))
                    if spans:
                        corrections += 1
            total_corrections += corrections
            print(f"  {'':12} → learned from {corrections} queries", flush=True)
