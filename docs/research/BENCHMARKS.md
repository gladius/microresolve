# ASV Router — Benchmark Comparison
...
| **ASV (all data + learning)** | **89.7%** | **50 μs** | **$0** | **Nothing** | This repo |
...
| **ASV (50 seeds, no learning)** | **80.4%** | **44 μs** | **$0** | **Nothing** | This repo |
...
| **ASV (10 seeds, no learning)** | **61.5%** | **33 μs** | **$0** | **Nothing** | This repo |
...
| **ASV (all data + learning)** | **95.1%** | **76 μs** | **$0** | **Nothing** | This repo |
...
| **ASV (120 seeds, no learning)** | **84.9%** | **76 μs** | **$0** | **Nothing** | This repo |
| **ASV (50 seeds, no learning)** | **80.4%** | **42 μs** | **$0** | **Nothing** | This repo |
| **ASV (10 seeds, no learning)** | **61.5%** | **25 μs** | **$0** | **Nothing** | This repo |

---

## The Scaling Curve

How accuracy changes as you feed ASV more data.
...
ASV decomposes this into 3 intents, preserves user ordering, and detects relationships.
...
ASV is **100-10,000x faster** than every alternative. At 130K queries/second, one core handles **11 billion queries/day**.
...
| **ASV Router** | **$0** | Runs on any machine. No model. No API. |

---

## Resource Footprint

| Metric | ASV Router | BERT | Qwen2.5-7B | GPT-4 API |
|--------|-----------|------|------------|-----------|
...
**ASV is not an LLM replacement.** It handles the 80% of traffic that doesn't need intelligence, at zero cost, in microseconds. The 20% that needs understanding goes to the LLM. Combined: better accuracy at lower cost than either alone.
...
```
  ...
  └──────────┘                        │  traffic)     │
       ...
       ▼
  ┌──────────┐    picks from           ┌──────────────┐
  │ LLM with │    ASV's top-3         │ Execute       │
  │ top-3     │ ──────────────────────▶│ (20% of       │
  │ (~500ms)  │    candidates          │  traffic)     │
  └──────────┘                        └──────────────┘
```

- **ASV top-3 accuracy is 94-95%** — the correct intent is almost always in the candidate list
...
```bash
git clone https://github.com/gladius/asv-router
cd asv-router
