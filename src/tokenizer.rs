...
/// Public wrapper for expand_contractions (used by Resolver for dual-path tokenization).
pub fn expand_contractions_public(text: &str) -> String {
    expand_contractions(text)
}

/// Pseudo-negation phrases that look like negation but aren't.
/// These bypass negation handling entirely (from NegEx algorithm, Chapman 2001).
const PSEUDO_NEGATION: &[&str] = &[
    "no problem", "no worries", "no wonder", "no way",
    "not sure", "not bad", "not only", "not just",
    "can not wait", "can not believe",
];

/// Detect word positions that are negated.
///
/// Returns a set of word indices that should receive the `not_` prefix
/// (NOT dropped — the terms survive with a negation marker).
///
...
fn tokenize_segment(text: &str) -> Vec<String> {
    ...
    // Bigrams (consecutive non-stop, non-CJK word pairs)
    // Negated terms participate in bigrams with their not_ prefix.
    // CJK bigrams are already generated by expand_cjk_run, so skip CJK words here
    let non_stop: Vec<String> = words.iter()
        .enumerate()
...
mod tests {
    ...
    fn tokenize_negation_prefix() {
        ...
        // No negation → normal behavior (no not_ prefix)
    ...
    fn tokenize_pseudo_negation_bypass() {
        // "no problem" is pseudo-negation — nothing should be prefixed
        let terms = tokenize("no problem with my order");
        assert!(!terms.iter().any(|t| t.starts_with("not_")), "pseudo-negation should not trigger not_ prefix");
        assert!(terms.contains(&"problem".to_string()));

        // "can't wait" expands to "can not wait" — contains pseudo "can not wait"?
        // Actually "can't" → "can not" is already treated as non-negation (inability),
        // so this is fine without pseudo-negation.
    ...
    fn tokenize_negation_in_multi_intent_context() {
        // "cancel my order but don't refund" — both terms should survive
        let terms = tokenize("cancel my order but don't refund");
        assert!(terms.contains(&"cancel".to_string()), "cancel should be bare (not negated)");
        assert!(terms.contains(&"order".to_string()));
        // After "but", negation resets. Then "don't refund" → not_refund
        assert!(terms.contains(&"not_refund".to_string()), "refund should be negated");
        assert!(!terms.contains(&"refund".to_string()), "bare refund should not appear");
    }
