...
//!   4. Built-in default
//!
//! Legacy env var names (`PORT`, `LLM_API_KEY`, `ASV_DATA_DIR`, `LLM_PROVIDER`,
//! `LLM_MODEL`) stay honored for backward compatibility with anyone who was
...
#[command(
    name = "microresolve",
    version,
    about = "Pre-LLM decision layer: intent routing, PII detection, safety filtering, tool prefiltering.",
...
pub struct Cli {
    /// Port to listen on (default: 3001). Overrides PORT env var and config file.
...
pub fn resolve(cli: &Cli) -> ResolvedConfig {
    ...
    let data_dir = cli
        .data
        .clone()
        .or_else(|| std::env::var("ASV_DATA_DIR").ok().map(PathBuf::from))
