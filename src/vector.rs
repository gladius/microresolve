...
pub struct LearnedVector {
    #[serde(alias = "seed_terms")]
    phrase_terms: HashMap<String, f32>,
...
impl LearnedVector {
    ...
    /// Get phrase terms (for persistence).
    ...
    pub fn score(&self, query_terms: &[String]) -> f32 {
        query_terms
            .iter()
            .map(|t| {
                let phrase = self.phrase_terms.get(t).copied().unwrap_or(0.0);
    ...
    pub fn effective_terms(&self) -> HashMap<String, f32> {
        let mut merged = self.phrase_terms.clone();
