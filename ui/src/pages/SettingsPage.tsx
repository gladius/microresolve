...
function GitSyncSection() {
  const [gitSettings, setGitSettings] = useState<{ remote_url: string | null; auto_push: boolean; has_repo: boolean } | null>(null);
  ...
  useEffect(() => {
    api.getGitSettings().then(s => {
      setGitSettings(s);
      setRemoteInput(s.remote_url ?? '');
    }).catch(() => {});
  }, []);
  ...
  const save = async () => {
    setSaving(true);
    try {
      const s = await api.putGitSettings(remoteInput.trim() || null);
      setGitSettings(s);
      setRemoteInput(s.remote_url ?? '');
      showToast('Git remote saved', true);
    } catch (e) {
      showToast('Save failed: ' + (e instanceof Error ? e.message : 'unknown'), false);
    }
    setSaving(false);
  };
  ...
  const push = async () => {
    setPushing(true);
    try {
      const r = await api.gitPushNow();
      if (r.ok) showToast('Push successful', true);
      else showToast('Push failed: ' + (r.error ?? 'unknown'), false);
    } catch (e) {
      showToast('Push failed: ' + (e instanceof Error ? e.message : 'unknown'), false);
    }
    setPushing(false);
  };

  const noRepo = gitSettings && !gitSettings.has_repo;
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <h2 className="text-lg font-semibold text-zinc-100">Data sync (Git)</h2>
        <p className="text-xs text-zinc-500 mt-1">
          Every change to your training data auto-commits to a git repo at the data directory.
          Set a remote here to also push each commit to a real repo (GitHub / GitLab / etc.).
        </p>
      </div>

      {noRepo ? (
        <div className="border border-zinc-700 rounded-lg p-4 text-sm text-zinc-500">
          Server is running in-memory; no data directory to sync.
        </div>
      ) : (
        <div className="space-y-4">
          <div>
            <label className="text-xs text-zinc-400 block mb-1.5">Remote URL</label>
            ...
            <p className="text-[11px] text-zinc-600 mt-1.5">
              Auth uses whatever git is configured with on the server (SSH agent, credential helper,
              or token-in-URL like https://x-access-token:TOKEN@...).
            </p>
          </div>

          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 text-sm">
              {gitSettings?.auto_push ? (
                <>
                  <span className="text-emerald-400">✓</span>
                  <span className="text-zinc-400">Remote configured — auto-push enabled</span>
                </>
              ) : (
                <>
                  <span className="text-amber-400">⚠</span>
                  <span className="text-zinc-400">No remote set — changes are committed locally only</span>
                </>
              )}
            </div>
            {gitSettings?.auto_push && (
