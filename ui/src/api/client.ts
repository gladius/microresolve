...
export const api = {
  ...
  addIntent: (
    ...
    if (Array.isArray(seeds)) {
      ...
      body.phrases_by_lang = seeds;
    }
    if (description !== undefined) body.description = description;
    return post<void>('/intents', body);
  },
  ...
  getModels: async (): Promise<NamespaceModel[]> => {
    const s = await get<{ models?: NamespaceModel[] }>('/settings');
    return s.models ?? [];
  },
  ...
  // Review
  report: (query: string, detected: string[], session_id?: string) =>
    post<{ id: number }>('/report', { query, detected, session_id }),
  ...
  reviewAnalyze: (id: number) =>
    post<FullReviewResult>('/review/analyze', { id }),
  ...
  getNamespaceHistory: (namespaceId: string, limit = 20) =>
    fetch(`${BASE}/namespaces/${encodeURIComponent(namespaceId)}/history?limit=${limit}`, { headers: appHeaders() })
      .then(r => r.json() as Promise<{ namespace_id: string; commits: { sha: string; ts: number; message: string; author: string }[] }>),
  rollbackNamespace: (namespaceId: string, sha: string) =>
    post<{ rolled_back_to: string; reloaded_namespaces: string[]; dropped_namespaces: string[] }>(
      `/namespaces/${encodeURIComponent(namespaceId)}/rollback`, { sha }
    ),

  // Semantic diff between two commits
  getNamespaceDiff: (namespaceId: string, from: string, to: string) =>
    fetch(`${BASE}/namespaces/${encodeURIComponent(namespaceId)}/diff?from=${from}&to=${to}`, { headers: appHeaders() })
      .then(async r => {
        if (!r.ok) throw new Error(await r.text());
        return r.json() as Promise<{
          from: string; to: string;
          intents_added: { id: string; phrases_sample: string[]; total_phrases: number }[];
          intents_removed: { id: string; phrases_sample: string[]; total_phrases: number }[];
  ...
  // Git settings
  getGitSettings: () => get<{ remote_url: string | null; auto_push: boolean; has_repo: boolean }>('/settings/git'),
  putGitSettings: (remote_url: string | null) =>
    fetch(`${BASE}/settings/git`, {
      method: 'PUT',
